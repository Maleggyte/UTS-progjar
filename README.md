...
```
pemetaan warna python pemetaan_warna  = { 'red': 'merah', 'yellow': 'kuning', 'blue': 'biru', 'orange': 'orange', 'purple': 'ungu', 'green': 'green', 'brown': 'coklat', 'black': 'hitam', 'white': 'putih' }  | Warna Inggris | Warna Indonesia | | --------------- | ------------------------ | | red | merah | | yellow | kuning | | blue | biru | | orange | orange | | purple | ungu | | green | hijau | | brown | coklat | | black | hitam | | white | putih |
...
```py
	...
	def  hentikan_server():
		...
		sys.exit(0) # Keluar dari program dengan kode keluar 0
```
fungsi pengiriman warna ke klien
```py
	def  kirim_warna_ke_klien(warna):
		for  alamat_klien  in  klien:
		SoketKlien.sendto(str.encode(warna), alamat_klien)
